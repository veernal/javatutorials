...
export class FindComponent{
  ...
  constructor(private _findService: FindService, private router: Router) {
    ...
    this.invoiceForm = new FormGroup({
      subscriptionId: new FormControl([Validators.min(0), isNaN]),
  ...
  findInvoice(subscriptionId: string, emailId: string) {
    this._findService.findBookBySubscriptionId(emailId, Number(subscriptionId)).subscribe({
      next: (res: any) => {
        if (!!res.statusCode) {
          alert(res.message)
        } else {
